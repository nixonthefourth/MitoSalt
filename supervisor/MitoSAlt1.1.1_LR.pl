...
my $p1 = $ARGV[1]; #this should now be a path to bam file, not fastq1
#my $p2 = $ARGV[2];
my $tag = $ARGV[2];
...
my $usage = "usage: perl mitsalt.pl <config_file> <bam file >  <study name>";
die "Configuration file missing, $usage" unless $config_file;
die "Bam file missing, $usage" unless $p1;
...
if($nu_mt eq 'yes' && $enriched eq 'no'){
  #print scalar(localtime).": copy alignment and sort\n";
  ...
  #system ("cp $p1 tmp_tmp_$tag.bam");
  #system ("cp $p1.bai tmp_tmp_$tag.bam.bai");
  #system ("samtools sort tmp_tmp_$tag.bam > tmp_$tag.bam");
  #system ("samtools index tmp_$tag.bam");
  #system ("rm tmp_tmp_$tag.bam");
  #system ("rm tmp_tmp_$tag.bam.bai");
  #EXTRACT READS
  ...
  system("$samtools idxstats $p1 > indel/$tag.count.txt");
  system("$samtools view -u -@ $threads -f 12 $p1 > tmp1_$tag.bam"); #flag to extract unmapped reads with mate also unmapped
  system("$samtools view -u -@ $threads $p1 chrM > tmp2_$tag.bam");
...
if($cn_mt eq 'yes' && $enriched eq 'no' && $o_mt eq 'yes' && $nu_mt eq 'yes'){
  ...
  system("$sambamba depth region -L tmp_random_$tag.bed -t $threads -o tmp_random_$tag.cov $p1");
